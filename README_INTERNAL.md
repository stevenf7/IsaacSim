...
| Checkbox | Effect |
|---|---|
| **User docs only change** | Skips builds, tests, and deploy stages. Only runs `check-code-format`, `check-docs-image-filenames`, `check-docs-unused-assets`, and the lightweight docs build (`build-docs-only-mr`). **Only check this if your changes are exclusively docs/user-guide content or related docs media.** |
...
`check-docs-linkcheck` is not controlled by a template checkbox. It appears as a non-blocking manual MR job, and scheduled/web pipelines can run it by setting `RUN_DOCS_LINKCHECK=true`.

## IsaacLab Integration Test Dashboard

An internal HTML dashboard that tracks IsaacLab integration test results across
Isaac Sim branches over time, including data from GitHub Actions workflows.

See [IsaacLab Test Dashboard](docs/internal/isaaclab_test_dashboard.md) for full
details on how it works, how to run it locally, and where results are published.
...
A Cursor skill in this repo can run builds, preview, and formatting. Use these prompts in Cursor:
- **"Build the docs"** — full docs build
- **"Build just the user guide"** — user guide only
- **"Rebuild my docs changes quickly"** — incremental user-guide rebuild after a successful full/user-guide docs build
- **"Build the API docs"** — API reference only
- **"Preview my docs changes"** — serve docs locally
- **"I'm ready to commit my docs changes"** — format and prepare for commit

Manual commands:

| Task | Linux | Windows |
|------|--------|---------|
| Full docs build | `./tools/build_docs.sh` | `tools\build_docs.bat` |
| User guide only | `./repo.sh docs --project isaac-sim -c release --warn-as-error=0` | `repo.bat docs --project isaac-sim -c release --warn-as-error=0` |
| Fast incremental user-guide rebuild | `./repo.sh docs --project isaac-sim -c release --stage sphinx --no-clean --no-install --warn-as-error=0` | `repo.bat docs --project isaac-sim -c release --stage sphinx --no-clean --no-install --warn-as-error=0` |
| Preview locally | `bash skills/_internal/build-docs/scripts/serve_docs.sh` | `skills\_internal\build-docs\scripts\serve_docs.bat` |
| Format before commit | `./format_code.sh` | `.\format_code.bat` |

**Output:** Built docs are in `_build/docs/isaac-sim/latest/`. The preview server serves them at http://localhost:8000 (user guide) and http://localhost:8000/py/ (API docs). Pass a port to avoid conflicts: `serve_docs.sh 9000` or `serve_docs.bat 9000`.

**Tip:** Pass extra arguments to the build scripts to forward to the underlying `repo.sh docs` command (e.g. `./tools/build_docs.sh --project isaac-sim` to build only the user guide).

**Fast iteration:** After an initial successful user-guide build, use the fast incremental command for RST/MD page edits. It runs only the Sphinx stage, keeps the existing output directory, and skips dependency installation. Use the normal user-guide build again after changing Sphinx config/theme files, generated API/extension docs inputs, or if the incremental output looks stale.

**Docs media CI checks:** Two lightweight MR jobs protect docs media. `check-docs-image-filenames` validates changed docs image/video filenames against the Isaac Sim naming convention. `check-docs-unused-assets` runs on every MR, but only checks added/copied/modified/renamed media under `docs/isaacsim/images`; existing unused assets do not fail the job unless the MR touches them.

To reproduce both checks locally against `develop`:
...
./docs/validate_filenames.sh --files-from /tmp/docs_only_changed_files.txt
python3 docs/tools/check_unused_assets/check_unused_assets.py --files-from /tmp/docs_changed_files.txt
```

If `check-docs-image-filenames` fails, rename the changed media file to the required `isim_<VERSION_NUM>_<APP_TYPE>_<DOC_TYPE>_<APP_VIEW>_<YOUR_FILE_NAME>.<ext>` pattern and update every RST reference. If `check-docs-unused-assets` fails, reference the changed media from an RST page, remove it from the MR, or move it outside `docs/isaacsim/images` if it is not user-guide media. The full manual scan (`./docs/check_unused_assets.sh`) may report existing backlog items; those are not MR blockers unless touched by the MR.

**Docs linkcheck:** `check-docs-linkcheck` is a non-blocking manual MR job and an opt-in scheduled/web job with `RUN_DOCS_LINKCHECK=true`. It uses Sphinx's built-in linkcheck builder through `./repo.sh docs --project isaac-sim --build linkcheck --stage sphinx --warn-as-error=0`; broken-link details are written to `_build/docs/isaac-sim/latest-linkcheck/output.txt` and uploaded with the CI artifacts. The legacy custom crawler under `docs/tools/linkcheck/` has been removed.

**Before committing:** Always run the formatter (`./format_code.sh` or `.\format_code.bat`). CI will fail if code or docs are not formatted.
