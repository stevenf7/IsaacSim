...
#. Manually set namespaces in the `nodeNamespace` field. 
...
#. Open a new stage. Open the Script Editor (``Window > Script Editor``), then add and run the following snippet. This will create a set of XForms that are arranged to mimic a robot articulation.
...
**All other OmniGraph nodes**: The path to the OmniGraph node is used to identify the path the namespace search algorithm takes and appends namespace values accordingly. In this case the location of these OmniGraph nodes is relevant. 
...
- Demonstrating how the ``isaac:namespace`` attribute can be set on prims on a robot to automatically generate namespaces
