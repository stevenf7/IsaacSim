...
.. _isaac_sim_install_python_anaconda:
...
.. warning::
    ...
    **This setup** (prior to Isaac Sim - Python packages installation) **is deprecated and will be removed in the next release**.
    To install/use |isaac-sim_short| on a Conda (or any other Python environment), see :ref:`isaac_sim_app_install_pip` instead.
...
#. You must set up environment variables so that |isaac-sim_short| Python packages are located correctly.  On Linux, you can do this as follows:
...
#. Run samples as follows in the ``isaac-sim`` Conda env:
    ...
    .. literalinclude:: ../snippets/installation/install_python/run_samples_as_follows_in_the_isaac_sim_conda_env.py
        :language: python
