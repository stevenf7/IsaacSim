...
============== ======================== =============================================
Ubuntu 24.04    Jazzy (recommended)      Default Linux installation
...
ROS 2 Jazzy on Ubuntu 24.04 is recommended. Verify or choose your configuration to continue:
...
.. config-selector::
   ...
   :persist: session
   :persist-key: ros2
   :options: platform=Linux|Windows,ubuntu_version=Ubuntu 24.04|Ubuntu 22.04,ros_distro=Jazzy|Humble,package_type=Default ROS Interfaces|Custom ROS Interfaces,install_method=Pixi|WSL2 (Deprecated)
   :dependencies: ubuntu_version=platform:Linux,package_type=platform:Linux;ubuntu_version:Ubuntu 22.04,install_method=platform:Windows
...
.. config-content::
   ...
   .. note::
      ...
      **Default ROS Interfaces** use standard ROS 2 message and service packages already provided by the selected ROS installation or |isaac-sim_short| internal ROS libraries.

      **Custom ROS Interfaces** use message or service types generated from your own ROS 2 packages. Isaac Sim loads those generated Python modules in its Python 3.12 runtime, so custom interface workspaces on Ubuntu 22.04 must also be built with Python 3.12.


.. _isaac_sim_app_install_ros_options:
...
.. config-content::
   ...
   .. note::
      ...
      ROS 2 Humble on Ubuntu 24.04 is not an officially supported configuration. Use ROS 2 Jazzy on Ubuntu 24.04, or use ROS 2 Humble on Ubuntu 22.04.
...
.. config-content::
   :show-when: platform=Windows,ros_distro=Humble,install_method=Pixi
...
.. config-content::
   :show-when: platform=Windows,ros_distro=Humble,install_method=WSL2 (Deprecated)
...
.. config-content::
   :show-when: platform=Windows,ros_distro=Jazzy,install_method=WSL2 (Deprecated)
...
.. config-content::
   ...
   The Pixi workspace installs ROS 2 Jazzy natively on Windows via `RoboStack <https://robostack.github.io/>`_ conda packages. WSL2 is not required, and all |isaac-sim_short| ROS 2 tutorials are supported.
   ...
   Continue to :ref:`isaac_sim_ros_workspace` to clone and set up the Pixi workspace.
...
.. config-content::
   ...
   To run the ROS 2 tutorials and examples, you must source your ROS 2 installation workspace in the terminal you plan to work in.

   #. To build the |isaac-sim_short| ROS workspaces, ensure you have followed :ref:`isaac_sim_app_install_ros_options`.
...
.. config-content::
   ...
   #. Follow the instructions in :ref:`isaac_ros_docker` to build and source the workspace using a ROS Docker container.
   ...
   .. config-content::
      ...
      **Custom ROS Interfaces**
      ...
      #. To run external nodes, use the Docker container as described in :ref:`isaac_ros_docker`.
...
.. config-content::
   ...
   #. To build the |isaac-sim_short| ROS workspaces, ensure you have followed :ref:`isaac_sim_app_install_ros_options` in WSL2.
...
.. config-content::
   ...
   #. Open a Command Prompt and navigate to the Jazzy workspace:

      .. code-block:: winbatch

         cd C:\\IsaacSim-ros_workspaces\\jazzy_ws

   #. Install all ROS 2 Jazzy packages and build dependencies (this may take several minutes on first run):

      .. code-block:: winbatch

         pixi install

   #. Build the workspace with MSVC:
      ...
      .. note:: After the first successful build, Pixi sources ``install\\setup.bat`` automatically whenever you enter ``pixi shell`` or invoke ``pixi run``. Your built packages are available without any manual sourcing.
   ...
   **Terminal 2 — Start Isaac Sim**
   ...
   The workspace exposes a ``sim`` task that launches |isaac-sim_short| with the ROS 2 bridge already enabled:
   ...
   **Adding dependencies**
   ...
   - Contribute the package to `ros-jazzy on RoboStack <https://github.com/RoboStack/ros-jazzy>`_ if it is missing from the default ROS channel.
...
.. config-content::
   ...
   **Running Standalone Scripts or Manually Specifying ROS 2 Internal Libraries**

   If you are using ``./python.sh`` to run standalone |isaac-sim_short| scripts, you must manually enable the internal libraries.

   To directly set a specific internal ROS 2 library, set the following environment variables in a new terminal before running |isaac-sim_short|. If |isaac-sim_short| is installed in a non-default location, replace the ``isaac_sim_package_path`` environment variable with the path to your |isaac-sim_short| installation root folder.

   - To run |isaac-sim_short| using manually selected ROS 2 internal libraries:
...
.. config-content::
   ...
   #. If you DID NOT follow :ref:`isaac_sim_ros_workspace_setup`, create a file named ``fastdds.xml`` under ``C:\.ros\``, paste the following snippet link into the file:
...
.. config-content::
   :show-when: platform=Windows,install_method=Pixi

   .. note:: Fast DDS is not applicable for Pixi-based installations. Pixi uses Zenoh middleware, which is supported. See :ref:`isaac_sim_app_install_zenoh`.
...
.. config-content::
   ...
   .. note:: Cyclone DDS is not applicable for Pixi-based installations. Pixi uses Zenoh middleware, which is supported. See :ref:`isaac_sim_app_install_zenoh`.
...
.. config-content::
   ...
   .. note:: Cyclone DDS is not applicable for Pixi-based installations. Pixi uses Zenoh middleware. See :ref:`isaac_sim_app_install_zenoh`.
...
.. config-content::
   ...
   The Pixi workspace uses Zenoh (``rmw_zenoh_cpp``) as the default middleware. Zenoh is pre-configured as part of the workspace setup. Refer to the :ref:`isaac_sim_ros_workspace` section above for running instructions.
...
.. config-content::
   ...
   .. note:: |isaac-sim_short| Zenoh interfaces do not come packaged with internal libraries. Install RMW Zenoh on your system and source it before running |isaac-sim_short|.
   ...
   **Installing and running Zenoh**
   ...
   #. In a **separate terminal**, source ROS 2 and start the Zenoh router. The router must be running before any ROS 2 nodes can discover each other:
      ...
      .. note:: Without the Zenoh router, nodes cannot discover each other because multicast discovery is disabled by default in the node's session config. Instead, nodes receive discovery information about other peers through the Zenoh router's gossip functionality.

   #. Before running |isaac-sim_short|, set the ``RMW_IMPLEMENTATION`` environment variable in the terminal where you will launch |isaac-sim_short|. Moving forward, if any examples show setting the environment variable to ``rmw_fastrtps_cpp``, you can replace it with the command below:
...
.. config-content::
   ...
   **Installing and running Zenoh**
   ...
   #. Ensure you have already cloned the |isaac-sim_short| ROS Workspace Repository from `<https://github.com/isaac-sim/IsaacSim-ros_workspaces>`_. If not, follow the steps in :ref:`isaac_sim_ros_workspace_setup`.
   ...
   #. In a **separate terminal**, start the Zenoh router. The router must be running before any ROS 2 nodes can discover each other.
      ...
      Use the Docker container from the :ref:`isaac_ros_docker` section. If you have not already started the ``ros_ws_docker`` container, follow the steps there first. Then, inside the container, install ``ros-jazzy-rmw-zenoh-cpp`` and start the router:
...
.. config-content::
         ...
         .. tab-set::
            ...
            .. tab-item:: aarch64 (DGX Spark)
               ...
               .. code-block:: bash
                  ...
                  docker run -it --net=host --env="DISPLAY" --env="ROS_DOMAIN_ID" -v ~/IsaacSim-ros_workspaces/jazzy_ws:/jazzy_ws --name ros_ws_docker arm64v8/ros:jazzy /bin/bash
   ...
   #. Inside the Docker container, set the ``FASTRTPS_DEFAULT_PROFILES_FILE`` environment variable as per instructions in :ref:`isaac_sim_app_enable_ros`.
