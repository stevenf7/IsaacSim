...
omni.kit.app.get_app().get_extension_manager().set_extension_enabled_immediate("omni.isaac.cortex_sync", True)
...
from omni.isaac.cortex.motion_commander import MotionCommand, ApproachParams, PosePq, close_gripper
...
from omni.isaac.cortex_sync.cortex_ros import cortex_init_ros_node, CortexControl, CortexSimRobot
...
class PeckContext(DfToolsContext):
    def __init__(self, tools):
        ...
        self.monitors = [PeckContext.monitor_active_target_p, PeckContext.monitor_gripper]
    ...
    def monitor_gripper(self):
        """ Constantly issue a close command. If we're already there, this does nothing.
        """
        self.tools.robot.gripper.close()
...
def main():
    cortex_init_ros_node("cortex_control_example")

    world = World()
    robot = world.scene.add(Franka(prim_path="/World/franka", name="franka"))
    robot_sim = world.scene.add(
        Franka(prim_path="/World/franka_sim", name="franka_sim", position=np.array([-2.0, 0.0, 0.0]))
    )
    ...
    cortex_sim = CortexSimRobot(robot_sim)
    cortex_control = CortexControl(robot, commander)
