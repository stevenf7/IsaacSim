...
def run_differential_control_loop(
    ...
    if not controller.reset(estimated_state, None, 0.0):
        print("ERROR: Controller reset failed!")
        return
    ...
    for step in range(num_steps):
        ...
        if desired_state is not None:
            ...
            # If controller returns None, print warning
            if step % 60 == 0:  # Print once per second
                print(f"  Warning: Controller returned None at step {step} (t={simulation_time:.2f}s)")

        # Print status periodically
        if step % 60 == 0:  # Every second
            current_positions = robot.get_dof_positions().numpy().flatten()
            current_velocities = robot.get_dof_velocities().numpy().flatten()
            print(f"\n  Step {step} (t={simulation_time:.2f}s):")
            print(f"    Left wheel:  pos={current_positions[0]:.3f}, vel={current_velocities[0]:.3f}")
            print(f"    Right wheel: pos={current_positions[1]:.3f}, vel={current_velocities[1]:.3f}")
            if setpoint_state is not None:
                v_lin = setpoint_state.root.linear_velocity.numpy()[0]
                v_ang = setpoint_state.root.angular_velocity.numpy()[2]
                print(f"    Setpoint:    v_linear={v_lin:.3f} m/s, v_angular={v_ang:.3f} rad/s")
...
def differential_drive_control(
    ...
    if use_filter:
        ...
        print("  Using SequentialController to apply low-pass filter to differential output")
        print(f"    Filter alpha: {filter_controller.alpha}")
    else:
        controller = differential_controller

    if add_noise:
        print("  Adding noise to root velocity setpoints")
