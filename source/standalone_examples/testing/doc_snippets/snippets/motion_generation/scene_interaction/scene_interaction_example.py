...
def demonstrate_scene_query():
    ...
    print("\n" + "=" * 60)
    print("SceneQuery: Finding Objects")
    print("=" * 60)
    ...
    print(f"\nFound {len(collision_objects)} collision objects:")
    for obj_path in collision_objects:
        print(f"  - {obj_path}")
    ...
    print(f"\nFound {len(filtered_objects)} objects (excluding ground plane):")
    for obj_path in filtered_objects:
        print(f"  - {obj_path}")

    # Find all robots in the stage (if any)
    robots = query.get_robots_in_stage()
    print(f"\nFound {len(robots)} robots in stage:")
    for robot_path in robots:
        print(f"  - {robot_path}")
...
def demonstrate_obstacle_strategy(obstacle_paths):
    ...
    print("\n" + "=" * 60)
    print("ObstacleStrategy: Representation Management")
    print("=" * 60)

    # Create an obstacle strategy
    strategy = mg.ObstacleStrategy()

    # Set default safety tolerance for all shapes
    strategy.set_default_safety_tolerance(0.05)  # 5cm padding
    print("\nSet default safety tolerance to 0.05m")

    # Set default representation for Mesh to OBB with large safety tolerance
    ...
    print("Set default Mesh representation to OBB with 0.15m safety tolerance")
    ...
    if mesh2_path in obstacle_paths:
        ...
        print(f"\nSet override for {mesh2_path}:")
        print(f"  Representation: TRIANGULATED_MESH (for faithful interaction)")
        print(f"  Safety tolerance: 0.01m (1cm)")

    if len(overrides) > 0:
        strategy.set_configuration_overrides(overrides)

        # Get configuration for the overridden mesh
        config = strategy.get_obstacle_configuration(mesh2_path)
        print(f"\nVerified configuration for {mesh2_path}:")
        print(f"  Representation: {config.representation}")
        print(f"  Safety tolerance: {config.safety_tolerance}")
...
class ExampleWorldInterface(WorldInterface):
    ...
    def __init__(self):
        ...
        self.obstacles = {}
        print("ExampleWorldInterface initialized")
    ...
    def add_spheres(self, prim_paths, radii, scales, safety_tolerances, poses, enabled_array):
        ...
        for i, path in enumerate(prim_paths):
            ...
            print(f"  Added sphere: {path} (radius={radius:.3f}m)")
    ...
    def add_cubes(self, prim_paths, sizes, scales, safety_tolerances, poses, enabled_array):
        ...
        for i, path in enumerate(prim_paths):
            ...
            print(f"  Added cube: {path} (size={size:.3f}m)")
    ...
    def add_triangulated_meshes(
        ...
        for i, path in enumerate(prim_paths):
            ...
            print(
                f"  Added triangulated mesh: {path} (safety_tolerance={safety_tolerance:.3f}m, {num_triangles} triangles)"
            )
    ...
    def add_oriented_bounding_boxes(
        ...
        for i, path in enumerate(prim_paths):
            ...
            print(f"  Added OBB: {path} (safety_tolerance={safety_tolerance:.3f}m)")
    ...
    def add_planes(
        ...
        for i, path in enumerate(prim_paths):
            ...
            print(
                f"  Added plane: {path} (axis={axis}, length={length:.3f}m, width={width:.3f}m, safety_tolerance={safety_tolerance:.3f}m)"
            )
    ...
    def update_obstacle_transforms(self, prim_paths, poses):
        ...
        for i, path in enumerate(prim_paths):
            if path in self.obstacles:
                ...
                # In a real implementation, you would update transforms in your planning library
...
def demonstrate_world_binding(obstacle_paths, obstacle_strategy):
    ...
    print("\n" + "=" * 60)
    print("WorldBinding: Synchronizing Scene to Planning Library")
    print("=" * 60)
    ...
    print("\nInitializing WorldBinding...")
    binding.initialize()
    print(f"WorldBinding initialized - planning library now has {len(world_interface.obstacles)} obstacles")
