...
def prefix_with_isaac_asset_server(relative_path):
    ...
    if assets_root_path is None:
        carb.log_error("Nucleus server not found, could not get Isaac Sim assets folder")
        return
...
def register_scatter_boxes(prim):
    ...
    plane_rot_euler_deg = quat_to_euler_angles(np.array(forklift_quat_xyzw), degrees=True)
...
def simulate_falling_objects(prim, num_sim_steps=200, num_boxes=8):
    ...
    # Change the collision aproximation to convex hull
    collision_api = UsdPhysics.CollisionAPI.Apply(pallet_prim)
    physx_collision_api = PhysxSchema.PhysxCollisionAPI.Apply(pallet_prim)
    pallet_mesh_collision_api = UsdPhysics.MeshCollisionAPI.Apply(pallet_prim)
    pallet_mesh_collision_api.GetApproximationAttr().Set("convexHull")
    # UsdPhysics.MeshCollisionAPI(pallet_prim).GetApproximationAttr().Set("convexHull")
    ...
    for i in range(num_boxes):
        ...
        physx_collision_api = PhysxSchema.PhysxCollisionAPI.Apply(box_prim)
        mesh_collision_api = UsdPhysics.MeshCollisionAPI.Apply(box_prim)
        mesh_collision_api.GetApproximationAttr().Set("convexHull")
        # UsdPhysics.MeshCollisionAPI(box_prim).GetApproximationAttr().Set("convexHull")
    ...
    for i in range(num_sim_steps):
        ...
        if last_box and np.linalg.norm(last_box.get_linear_velocity()) < 0.001:
            print(f"Simulation stopped early after {i} steps")
            break
