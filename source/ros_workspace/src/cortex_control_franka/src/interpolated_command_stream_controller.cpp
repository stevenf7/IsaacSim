...
bool InterpolatedCommandStreamController::init(hardware_interface::RobotHW* robot_hardware, ros::NodeHandle& node_handle)
{
    ...
    {
        ROS_ERROR(
            "InterpolatedCommandStreamController: Error getting position joint "
    ...
    for (size_t i = 0; i < joint_names.size(); ++i)
    {
        ...
        {
            ROS_ERROR_STREAM("InterpolatedCommandStreamController: Exception getting joint handles: " << e.what());
            return false;
        }
    }
    ...
    command_stream_interpolator_ =
        cortex::control::LoadCommandStreamInterpolatorFromYaml(command_stream_interpolator_config);
    return true;
}
...
void InterpolatedCommandStreamController::update(ros::Time const& time, ros::Duration const& period)
{
    ...
    send_position_command(
        command_stream_interpolator_->NextCommand(controller_time_, current_position(), &is_interpolator_active));
    ...
    if (time >= next_print_time_)
    {
        std::cout << std::setprecision(10) << "[franka] time: " << (time - start_time_).toSec()
                  ...
                  << ", now: " << (ros::Time::now() - start_time_).toSec() << ", period: " << period.toSec()
                  << std::endl;
...
PLUGINLIB_EXPORT_CLASS(cortex::control::franka::InterpolatedCommandStreamController, controller_interface::ControllerBase)
