    ...
    const YAML::Node& command_stream_interpolator_config, bool verbose)
{
    ...
    auto rmpflow_command_interpolated_topic = util::GetOrDie<std::string>(command_topics, "interpolated");
    ...
    stream_interpolator->Init(ros::Duration(interpolation_delay), use_smoothing_interpolator, rmpflow_command_topic,
                              rmpflow_command_ack_topic, rmpflow_command_suppress_topic,
                              rmpflow_command_interpolated_topic, ros::Duration(blending_duration), backend_timeout);
