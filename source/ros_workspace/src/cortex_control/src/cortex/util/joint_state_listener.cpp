...
void JointStateListener::Init(const std::string& topic, const std::vector<std::string>& required_joints, int poll_rate)
{
...
{
    Init(topic, std::vector<std::string>());
}
...
void JointStateListener::Init(const std::string& topic, const std::vector<std::string>& required_joints)
{
                              ...
                              int poll_rate)
{
    ...
    subscriber_ = node_handle_.subscribe(topic,
                                         10, // Queue size.
                                         &JointStateListener::Callback, this);
...
void JointStateListener::InitWithZero(const std::string& topic, const std::vector<std::string>& required_joints)
{
    required_joints_ = required_joints;
    subscriber_ = node_handle_.subscribe(topic,
                                         10, // Queue size.
...
void JointStateListener::Callback(const sensor_msgs::JointState& joint_states)
{
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        current_state_map_[joint_states.name[i]] =
            SingleJointState(joint_states.position[i], joint_states.velocity[i],
                             has_efforts ? joint_states.effort[i] : 0., joint_states.header.stamp);
    }
    if (!is_available_)
    {
...
const std::unordered_map<std::string, SingleJointState>& JointStateListener::current_state_map() const
{
    return current_state_map_;
}

std::unordered_map<std::string, SingleJointState> JointStateListener::current_state_map_atomic() const
{
    std::lock_guard<std::mutex> guard(msg_mutex_);
    return current_state_map_;
}

std::vector<double> JointStateListener::CurrentPositions(const std::vector<std::string>& names) const
{
    std::lock_guard<std::mutex> guard(msg_mutex_);
...
StampedState JointStateListener::CurrentState() const
{
    ...
    for (uint32_t i = 0; i < required_joints_.size(); ++i)
    {
        ...
        ROS_ASSERT_MSG(access_iter != current_state_map_.end(), "Required joint not found: %s", name.c_str());
...
bool JointStateListener::IsReady() const
{
    return is_available();
}
...
std::unordered_map<std::string, SingleJointState> ToMap(const sensor_msgs::JointState& joint_states)
{
    auto n = joint_states.name.size();
    ROS_ASSERT(joint_states.position.size() == n);
    ROS_ASSERT(joint_states.velocity.size() == n);
    ...
    for (uint32_t i = 0; i < n; ++i)
    {
        js_map[joint_states.name[i]] = SingleJointState(
            joint_states.position[i], joint_states.velocity[i], joint_states.effort[i], joint_states.header.stamp);
    }
    return js_map;
}

std::vector<double> ExtractNamedPositions(const std::unordered_map<std::string, SingleJointState>& jstates,
