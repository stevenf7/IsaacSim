...
public:
    ...
    bool AddPt(double t, const PosVelAcc<typename interp_t::VectorXx>& p, std::string* error_str = nullptr) override
    {
        if (is_first_)
        {
            prev_eval_t_ = t;
        ...
        if (t <= prev_eval_t_)
        {
            ...
            {
                std::stringstream ss;
                ss << "Add time must be beyond the last eval time = " << t << " vs last eval t = " << prev_eval_t_;
                *error_str += ss.str();
            }
            return false;
        }
    ...
    bool Eval(double t, PosVelAcc<typename interp_t::VectorXx>& ret, std::string* error_str) const override
    {
        if (!IsReady(t))
        {
            if (error_str)
    ...
    {
        return is_ready_ && (t >= interpolator_.t0());
    }
