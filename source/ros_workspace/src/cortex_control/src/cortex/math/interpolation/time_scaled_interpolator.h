...
public:
    ...
    TimeScaledInterpolator(double t0,
        ...
        : t0_(t0),
          ...
          p1_(p1),
          time_range_(t1 - t0),
          scaled_traj_(traj_t(p0.Scale(time_range_), p1.Scale(time_range_), validate_interpolation_evals), time_range_),
    ...
    PosVelAcc<VectorXx> Eval(double t) const
    {
        ...
        ROS_ASSERT_MSG(scaled_traj_.Eval((t - t0_) / time_range_, ret, &error_str), "%s", error_str.c_str());
        return ret;
    }
    ...
    const PosVelAcc<VectorXx>& p0() const
    {
        return p0_;
    }
    double t1() const
    ...
    const PosVelAcc<VectorXx>& p1() const
    {
        return p1_;
    }
