...
struct PosVelAcc
{
    ...
    PosVelAcc<vec_t> Scale(double dt) const
    {
        return PosVelAcc<vec_t>(x, dt * xd, (dt * dt) * xdd);
    }
...
template <class vec_t>
PosVelAcc<vec_t>::PosVelAcc(const vec_t& x, const vec_t& xd, const vec_t& xdd) : x(x), xd(xd), xdd(xdd)
{
