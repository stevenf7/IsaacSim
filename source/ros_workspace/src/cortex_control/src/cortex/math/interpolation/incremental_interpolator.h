...
{
public:
    explicit IncrementalInterpolator(bool prune_history = true, bool validate_interpolation_evals = true);
    ...
    // evaluations.
    bool AddPt(double t, const PosVelAccXd& p, std::string* error_str = nullptr) override;
    ...
    // false otherwise.
    bool Eval(double t, PosVelAccXd& ret, std::string* error_str = nullptr) const override;
    using Interpolator<Eigen::VectorXd>::Eval;

    int num_intervals() const
    {
        return segment_interpolators_.size();
    }
