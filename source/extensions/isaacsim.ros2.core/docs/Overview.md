...
The extension integrates with the Carbonite framework for plugin loading, **omni.kit** for extension lifecycle management, and the ROS 2 Client Library (rcl) at the C API level. It serves as the foundation for all ROS 2 OmniGraph nodes and bridge components in Isaac Sim. The Python layer integrates with **isaacsim.core.utils** for render product and transformation queries, **isaacsim.sensors.camera** for camera attribute maps, and **isaacsim.core.simulation_manager** for test fixtures.
