...
public:
    ...
    static bool compute(OgnIsaacReadTimesDatabase& db)
    {
        ...
        { // read the data from fabric
            ...
            db.outputs.frameNumber() = frameIdentifier.type == omni::usd::hydra::FrameIdentifier::Type::eNoFrameNumber ?
                                           0 :
            ...
            if (inPostRenderGraph)
            {
                ...
                omni::usd::hydra::HydraRenderVar* newRenderVars =
                    new omni::usd::hydra::HydraRenderVar[renderProduct->renderVarCnt + 1];
                const size_t renderVarArraySize = sizeof(omni::usd::hydra::HydraRenderVar) * renderProduct->renderVarCnt;
