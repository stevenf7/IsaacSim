...
class ComboBoxField(UIWidgetWrapper):
    """
    Create a labeled ComboBox UI element.

    Args:
        label (str): Short descriptive text to the left of the ComboBox.
        items (List[str]): Items to populate the ComboBox.
        tooltip (str, optional): Text to appear when the mouse hovers over the label. Defaults to "".
        default_index (int, optional): Default selected index. Defaults to 0.
        on_selection_fn (Callable, optional): Callback when a new item is selected. Receives selected string.
            The return value is not used. Defaults to None.
        add_flourish (bool, optional): Whether to render the end-of-row flourish. Defaults to True.
    """
    ...
    def __init__(
        ...
        self._default_index = int(default_index) if default_index is not None else 0
        if self._default_index < 0:
            self._default_index = 0
        if len(self._items) > 0 and self._default_index >= len(self._items):
            self._default_index = 0
    ...
    def get_selection(self) -> typing.Optional[str]:
    ...
    def set_items(self, items: List[str], select_index: int = 0):
        self._items = list(items)
        self.combobox.model = DynamicComboBoxModel(self._items)
        if 0 <= select_index < len(self._items):
            self.combobox.model.get_item_value_model().set_value(select_index)

    def set_selection_by_index(self, index: int):
        if 0 <= index < len(self._items):
            self.combobox.model.get_item_value_model().set_value(index)

    def set_selection(self, selection: str):
        if selection in self._items:
            self.set_selection_by_index(self._items.index(selection))

    def set_on_selection_fn(self, on_selection_fn: Callable):
        self._on_selection_fn = on_selection_fn

    def _item_changed_fn_wrapper(self, model, val):
        if self._on_selection_fn is not None and len(self._items) > 0:
            idx = model.get_item_value_model().as_int
            if 0 <= idx < len(self._items):
                self._on_selection_fn(self._items[idx])
    ...
    def _create_ui_widget(self, label: str, tooltip: str):
        ...
        with containing_frame:
            with ui.HStack():
                ...
                if 0 <= self._default_index < len(items):
                    self.combobox.model.get_item_value_model().set_value(self._default_index)
