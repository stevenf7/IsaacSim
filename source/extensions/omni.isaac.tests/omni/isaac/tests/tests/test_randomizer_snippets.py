...
class TestRandomizerSnippets(omni.kit.test.AsyncTestCase):
    ...
    async def test_randomizing_a_light_source(self):
        ...
        def sphere_lights(num):
            ...
            for i in range(num):
                ...
                light_prim.CreateAttribute("inputs:radius", Sdf.ValueTypeNames.Float).Set(1.0)
        ...
        async def run_randomizations_async(num_frames, lights, delay=0):
            for _ in range(num_frames):
                for light in lights:
                    light.GetAttribute("xformOp:translate").Set(
                        (np.random.uniform(-3, 3), np.random.uniform(-3, 3), np.random.uniform(-3, 3))
                    )
                    light.GetAttribute("xformOp:scale").Set(
                        (np.random.uniform(5, 10), np.random.uniform(5, 10), np.random.uniform(5, 10))
                    )
                    light.GetAttribute("inputs:colorTemperature").Set(np.random.normal(6500, 500))
                    light.GetAttribute("inputs:intensity").Set(np.random.normal(35000, 5000))
    ...
    async def test_randomizing_textures(self):
        ...
        for _ in range(num_cubes):
            ...
            UsdGeom.Xformable(cube).AddTranslateOp().Set((np.random.uniform(-10, 10), np.random.uniform(-10, 10), 2))
            add_update_semantics(cube, "cube", "class")
        ...
        async def run_randomizations_async(num_frames, materials, textures, delay=0):
