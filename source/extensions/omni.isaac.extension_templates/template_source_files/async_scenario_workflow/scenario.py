...
class Scenario(ScenarioTemplate):
    ...
    def setup_scenario(self, articulation):
        ...
        self._object = FixedCuboid(
            "/Scenario/cuboid",
            position=np.array([self._object_radius, 0, self._object_height]),
            size=0.05,
    ...
    def update_scenario(self, step: float):
        ...
        x = self._object_radius * np.cos(self._time * self._object_frequency * 2 * np.pi)
        y = self._object_radius * np.sin(self._time * self._object_frequency * 2 * np.pi)

        self._object.set_world_pose(np.array([x, y, self._object_height]))
