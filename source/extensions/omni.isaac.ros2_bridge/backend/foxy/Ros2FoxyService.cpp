...
Ros2ServiceFoxy::Ros2ServiceFoxy(Ros2NodeBase* node, const char* service_name, const void* type, const Ros2QoSProfile& qos)
    : mNode(node), wait_set_initialized(false)
{
    mService = std::shared_ptr<rcl_service_t>(new rcl_service_t,
                                              [node](rcl_service_t* service)
                                              {
                                                  ...
                                                  if (RCL_RET_OK != ret)
                                                  {
                                                      RCL_ERROR_MSG(Ros2ServiceFoxy, rcl_service_fini);
                                                  }
                                                  delete service;
                                              });
    ...
    if (rc != RCL_RET_OK)
    {
        RCL_ERROR_MSG(Ros2ServiceFoxy, rcl_service_init);
        mService.reset();
        return;
    }
}
Ros2ServiceFoxy::~Ros2ServiceFoxy()
{
    if (wait_set_initialized)
    {
        ...
        if (rc != RCL_RET_OK)
        {
            RCL_ERROR_MSG(~Ros2ServiceFoxy, rcl_wait_set_fini);
        }
...
bool Ros2ServiceFoxy::getRequest(void* req_message)
{
    if (!wait_set_initialized)
    {
...
bool Ros2ServiceFoxy::sendResponse(void* res_message)
{
    if (!wait_set_initialized)
        return false;
