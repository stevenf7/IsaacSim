...
#include <rcl/rcl.h>
Ros2SubscriberFoxy::Ros2SubscriberFoxy(Ros2NodeBase* node, const char* topic_name, const void* type)
    : mNode(node), wait_set_initialized(false)
{
    mSub = std::shared_ptr<rcl_subscription_t>(new rcl_subscription_t,
                                               [node](rcl_subscription_t* subscription)
                                               {
                                                   ...
                                                   if (RCL_RET_OK != ret)
                                                   {
                                                       RCL_ERROR_MSG(Ros2SubscriberFoxy, rcl_subscription_fini);
                                                   }
                                                   delete subscription;
                                               });
    ...
    if (rc != RCL_RET_OK)
    {
        RCL_ERROR_MSG(Ros2SubscriberFoxy, rcl_subscription_init);
        mSub.reset();
        return;
    }
}
Ros2SubscriberFoxy::~Ros2SubscriberFoxy()
{

    rcl_ret_t rc = rcl_wait_set_fini(&wait_set);
    if (rc != RCL_RET_OK)
    {
        RCL_ERROR_MSG(~Ros2SubscriberFoxy, rcl_wait_set_fini);
    }
...
bool Ros2SubscriberFoxy::spin(void* ros_message)
{
