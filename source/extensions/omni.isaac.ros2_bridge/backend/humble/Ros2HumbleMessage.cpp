...
Ros2AckermannDriveStampedMessageHumble::Ros2AckermannDriveStampedMessageHumble()
    : Ros2BackendHumble("ackermann_msgs", "msg", "AckermannDriveStamped")
{
    msg = create();
}
const void* Ros2AckermannDriveStampedMessageHumble::getTypeSupportHandle()
{
    return getTypeSupportHandleDynamic();
}

void Ros2AckermannDriveStampedMessageHumble::getData(std::string& frameId,
                                                     double& timeStamp,
...
void Ros2AckermannDriveStampedMessageHumble::fillHeader(const double timestamp, const std::string& frame_id)
{
    if (!msg)
        return;

    ackermann_msgs__msg__AckermannDriveStamped* drive_msg = static_cast<ackermann_msgs__msg__AckermannDriveStamped*>(msg);
    Ros2BackendHumble::set_header(frame_id, static_cast<int64_t>(timestamp * 1e9), drive_msg->header);
}

void Ros2AckermannDriveStampedMessageHumble::fillData(const double& steeringAngle,
