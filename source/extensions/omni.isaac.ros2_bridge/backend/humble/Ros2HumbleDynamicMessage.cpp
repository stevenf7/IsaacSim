...
Ros2DynamicMessageHumble::Ros2DynamicMessageHumble(std::string pkgName, std::string msgSubfolder, std::string msgName)
    : Ros2BackendHumble(pkgName, msgSubfolder, msgName)
{
    // create message
    msg = create();
    // get message fields
...
{
    auto messageMembers = reinterpret_cast<const rosidl_typesupport_introspection_c__MessageMembers_s*>(members);
                                                          ...
                                                          const std::shared_ptr<std::vector<std::string>> messageValues)
{
    ...
    if (member->is_upper_bound_ || !member->array_size_)
    {
        ...
        auto embeddedMembers =
            reinterpret_cast<const rosidl_typesupport_introspection_c__MessageMembers_s*>(member->members_->data);
