...
rmw_time_t convertRos2QosTimeToHumble(const Ros2QoSTimeType& ros2Time)
{
    return { ros2Time.sec, ros2Time.nsec };
}

rmw_qos_profile_t Ros2QoSProfileHumbleConverter::convert(const Ros2QoSProfile& qos)
{
    ...
    profile.deadline = convertRos2QosTimeToHumble(qos.deadline);
    profile.lifespan = convertRos2QosTimeToHumble(qos.lifespan);
    profile.liveliness = ros2QoSLivelinessMap.at(qos.liveliness);
    profile.liveliness_lease_duration = convertRos2QosTimeToHumble(qos.livelinessLeaseDuration);
