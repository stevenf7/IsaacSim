...
void RosDifferentialBase::pubCallback(rclcpp::PublisherBase* pub)
{
    ...
    if (mUseSimTime)
    {
        odomMsg.header.stamp = rclcpp::Time(mTimeNanoSeconds);
    }
    else
    {
        odomMsg.header.stamp = rclcpp::Time(mSystemTimeNanoSeconds);
    }
    ...
    static_cast<rclcpp::Publisher<nav_msgs::msg::Odometry, std::allocator<void>>*>(pub)->publish(odomMsg);
}
...
void RosDifferentialBase::subCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
