...
static physx::PxVehicleWheels* getVehicleNoCheck(const pxr::SdfPath& vehiclePath,
                                                 carb::physics::PhysX* physxInterface,
                                                 size_t& vehicleId)
{
    vehicleId = physxInterface->getObjectId(vehiclePath, carb::physics::PhysXType::ePTVehicle);
...
template <typename T>
static void cacheWheelIndices(std::vector<T>& wheelCacheList, const size_t vehicleId, carb::physics::PhysX* physXInterface)
{
    ...
    for (uint32_t i = 0; i < wheelCount; i++)
    {
        ...
        const int32_t index = physXInterface->getWheelIndex(vehicleId, wheelCache.usdPath);
        if (index >= 0)
        {
...
VehicleSimulator::VehicleSimulator() : IsaacComponent()
{
    ...
    // mPhysxVehiclePtr = carb::getFramework()->acquireInterface<carb::physics::PhysXVehicle>();
    // if (!mPhysxVehiclePtr)
    // {
...
void VehicleSimulator::tick()
{
    ...
    auto command_composite_proto = commandComposite.initProto();
    {
        ...
        if (receive(mInputComponent, mCommandChannelName, header, command_composite_proto, buffers))
        {
            ...
            for (int i = 0; i < mAveragedAcceleration.size(); i++)
            {
            ...
            mCurrentSteeringAngle = pxr::GfClamp(elements[1], -maxSteerAngle, maxSteerAngle);
...
void VehicleSimulator::fillCache()
{
    ...
    if (mVehiclePrim && mVehiclePrim.HasAPI<pxr::PhysxSchemaPhysxVehicleAPI>())
    {
        ...
        if (mVehiclePrim.HasAPI<pxr::PhysicsSchemaMassAPI>())
        {
            pxr::PhysicsSchemaMassAPI massAPI(mVehiclePrim);
