...
DcContext* createContext()
{
    ...
    carb::physics::PhysX* physx = framework->acquireInterface<carb::physics::PhysX>();
    if (!physx)
    {
    ...
    if (scenePath)
    {
        scene = (PxScene*)ctx->physx->getPhysXPtr(SdfPath(scenePath), carb::physics::ePTScene);
...
DcHandle DcContext::registerD6Joint(const pxr::SdfPath& usdPath)
{
    ...
    PxD6Joint* joint = (PxD6Joint*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTJoint);
    if (joint)
    {
...
bool DcContext::refreshPhysicsPointers(DcRigidBody* body, bool verbose)
{
    ...
    PxActor* pxActor = (PxActor*)physx->getPhysXPtr(body->path, carb::physics::ePTActor);
    if (pxActor)
    {
    ...
    {
        PxArticulationLink* link = (PxArticulationLink*)physx->getPhysXPtr(body->path, carb::physics::ePTLink);
        if (link)
        {
...
bool DcContext::refreshPhysicsPointers(DcJoint* joint, bool verbose)
{
    ...
    PxArticulationJointReducedCoordinate* pxArticulationJoint =
        (PxArticulationJointReducedCoordinate*)physx->getPhysXPtr(joint->path, carb::physics::ePTLinkJoint);
...
bool DcContext::refreshPhysicsPointers(DcAttractor* att, bool verbose)
{
    ...
    PxJoint* pxJoint = (PxJoint*)physx->getPhysXPtr(att->path, carb::physics::ePTJoint);
...
DcObjectType CARB_ABI DcPeekObjectType(const char* usdPath)
{
    ...
    carb::physics::PhysX* physx = ctx->physx;

    // check if it's an articulation
    PxArticulationBase* abase = (PxArticulationBase*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTArticulation);
    if (abase)
    {
    ...
    PxActor* actor = (PxActor*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTActor);
    if (actor)
    {
    ...
    // check if it's an articulation link
    PxArticulationLink* link = (PxArticulationLink*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTLink);
    if (link)
    {
        return eDcObjectRigidBody;
    }
    ...
    // check if it's a d6joint
    PxD6Joint* d6joint = (PxD6Joint*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTJoint);
    if (d6joint)
    {
    ...
    PxJoint* joint = (PxJoint*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTJoint);
    if (joint)
    {
    ...
    // check if it's an articulation joint
    PxArticulationJointReducedCoordinate* artJoint =
        (PxArticulationJointReducedCoordinate*)physx->getPhysXPtr(SdfPath(usdPath), carb::physics::ePTLinkJoint);
    if (artJoint)
    {
        return eDcObjectJoint;
    }
...
DcDofState* CARB_ABI DcGetArticulationDofStateDerivatives(DcHandle artHandle, const DcDofState* states, const float* efforts)
{
    ...
    art->pxArticulation->applyCache(*(art->pxArticulationCache), PxArticulationCache::eFORCE |
                                                                     PxArticulationCache::ePOSITION |
                                                                     PxArticulationCache::eVELOCITY);
...
bool CARB_ABI DcSetArticulationDofStates(DcHandle artHandle, const DcDofState* states, DcStateFlags flags)
{
    ...
    */
    ...
    if (flags & kDcStatePos)
    {
        pxFlags |= PxArticulationCache::ePOSITION;
        for (int i = 0; i < numDofs; i++)
        {
    ...
    if (flags & kDcStateVel)
    {
        pxFlags |= PxArticulationCache::eVELOCITY;
        for (int i = 0; i < numDofs; i++)
        {
    ...
    art->pxArticulation->applyCache(*art->pxArticulationCache, PxArticulationCache::eALL); // hmmm
...
bool CARB_ABI DcSetDofState(DcHandle dofHandle, const DcDofState* state)
{
    ...
    if (dof && dof->art && state)
    {
        ...
        if (art->refreshCache())
        {
...
#if 0
            art->pxArticulation->applyCache(*art->pxArticulationCache, PxArticulationCache::ePOSITION | PxArticulationCache::eVELOCITY);
#else
            ZeroArray(art->pxArticulationCache->jointForce, art->pxArticulation->getDofs());
            ZeroArray(art->pxArticulationCache->jointAcceleration, art->pxArticulation->getDofs());
            ZeroArray(art->pxArticulationCache->linkVelocity, art->pxArticulation->getNbLinks());
            ...
            art->pxArticulation->applyCache(*art->pxArticulationCache, PxArticulationCache::eALL);
#endif
            return true;
        }
    }
...
#if 0
            art->pxArticulation->applyCache(*art->pxArticulationCache, PxArticulationCache::ePOSITION);
#else
            ZeroArray(art->pxArticulationCache->jointForce, art->pxArticulation->getDofs());
            ZeroArray(art->pxArticulationCache->jointAcceleration, art->pxArticulation->getDofs());
            ZeroArray(art->pxArticulationCache->linkVelocity, art->pxArticulation->getNbLinks());
...
#if 0
            art->pxArticulation->applyCache(*art->pxArticulationCache, PxArticulationCache::eVELOCITY);
#else
            ZeroArray(art->pxArticulationCache->jointForce, art->pxArticulation->getDofs());
            ZeroArray(art->pxArticulationCache->jointAcceleration, art->pxArticulation->getDofs());
            ZeroArray(art->pxArticulationCache->linkVelocity, art->pxArticulation->getNbLinks());
