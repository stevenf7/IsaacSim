...
static void inertiaToUrdf(const Matrix33& inertia, UrdfInertia& urdfInertia)
{
...
static void urdfToInertia(const UrdfInertia& urdfInertia, Matrix33& inertia)
{
...
static bool collapseFixedJoints(UrdfRobot& robot)
{
...
static Vec3 urdfAxisToVec(const UrdfAxis& axis)
{
    return { axis.x, axis.y, axis.z };
}

static std::string resolveXrefPath(const std::string& assetRoot, const std::string& urdfPath, const std::string& xrefpath)
{
...
static std::string GetNewSdfPathString(pxr::UsdStageWeakPtr stage, std::string path, int nameClashNum = -1)
{
