          ...
          int maxRays)
{
    ...
    while (startList.size() > 0 && count < maxRays)
    {
        if (count % 10000 == 0)
        {
            printf("Processing occupancy map: points left %d, points processed: %d\n", startList.size(), count);
        }
...
void CARB_ABI
GenerateMap(float gridResolution, float rayResolution, float minSearchDistance, float occupancyThreshold, int maxRays)
{
    ...
    physx::PxShape* shape1 = physxScenePtr->getPhysics().createShape(physx::PxPlaneGeometry(), *defaultMaterial, false);
    ...
    physx::PxShape* shape2 = physxScenePtr->getPhysics().createShape(physx::PxPlaneGeometry(), *defaultMaterial, true);
    ...
    physx::PxShape* shape3 = physxScenePtr->getPhysics().createShape(physx::PxPlaneGeometry(), *defaultMaterial, true);
    ...
    physx::PxShape* shape4 = physxScenePtr->getPhysics().createShape(physx::PxPlaneGeometry(), *defaultMaterial, true);
