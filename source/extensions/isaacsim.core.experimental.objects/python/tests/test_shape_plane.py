...
"""Validate Plane wrapping and plane-specific USD attributes.

The suite authors existing plane prims for wrap-mode tests, verifies USD geom
binding and collection length, and round-trips width, length, and axis tokens
...
async def populate_stage(max_num_prims: int, operation: Literal["wrap", "create"], **kwargs: Any) -> None:
    """Create a fresh stage and author existing plane prims for wrap-mode tests."""
    # create new stage
...
class TestPlane(omni.kit.test.AsyncTestCase):
    """Exercise Plane geometry checks and authored shape attributes."""
