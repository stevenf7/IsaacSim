...
"""Validate CylinderLight wrapping and cylinder-specific USD attributes.

The suite authors existing cylinder light prims for wrap-mode tests, verifies
type detection and collection length, and round-trips length, radius, and
treat-as-line state through indexed get/set calls.
"""
...
async def populate_stage(max_num_prims: int, operation: Literal["wrap", "create"], **kwargs: Any) -> None:
    """Create a fresh stage and author existing cylinder lights for wrap-mode tests."""
    # create new stage
...
class TestCylinderLight(omni.kit.test.AsyncTestCase):
    """Exercise CylinderLight type checks and authored attributes."""
    ...
    @parametrize(backends=["usd"], prim_class=TargetLight, populate_stage_func=populate_stage)
    async def test_lengths(self, prim: Any, num_prims: Any, device: Any, backend: Any) -> None:
        """Test lengths."""
    ...
    @parametrize(backends=["usd"], prim_class=TargetLight, populate_stage_func=populate_stage)
    async def test_enabled_treat_as_lines(self, prim: Any, num_prims: Any, device: Any, backend: Any) -> None:
