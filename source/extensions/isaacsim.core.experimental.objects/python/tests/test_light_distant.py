...
"""Validate DistantLight wrapping and distant-light USD attributes.

The suite authors existing distant light prims for wrap-mode tests, verifies
type detection and collection length, and round-trips angle values through
...
async def populate_stage(max_num_prims: int, operation: Literal["wrap", "create"], **kwargs: Any) -> None:
    """Create a fresh stage and author existing distant lights for wrap-mode tests.
...
class TestDistantLight(omni.kit.test.AsyncTestCase):
    """Exercise DistantLight type checks and angle authoring."""
    ...
    @parametrize(backends=["usd"], prim_class=TargetLight, populate_stage_func=populate_stage)
    async def test_angles(self, prim: Any, num_prims: Any, device: Any, backend: Any) -> None:
        """Test angles.
