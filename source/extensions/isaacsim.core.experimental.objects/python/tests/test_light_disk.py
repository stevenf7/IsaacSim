...
"""Validate DiskLight wrapping and disk-specific USD attributes.

The suite authors existing disk light prims for wrap-mode tests, verifies type
detection and collection length, and round-trips radius values through indexed
get/set calls.
"""
...
async def populate_stage(max_num_prims: int, operation: Literal["wrap", "create"], **kwargs: Any) -> None:
    """Create a fresh stage and author existing disk lights for wrap-mode tests."""
    # create new stage
...
class TestDiskLight(omni.kit.test.AsyncTestCase):
    """Exercise DiskLight type checks and radius authoring."""
