...
class LookAtCommander:
    """Simple look at commander that causes dofbot to look at a given location
    """

    def __init__(self, dofbot):
        self.dofbot = dofbot

    def go_pos(self, grip_pos, target_pos):
        self.dofbot.end_effector.look_at(
            Gf.Vec3f(grip_pos[0], grip_pos[1], grip_pos[2]), Gf.Vec3f(target_pos[0], target_pos[1], target_pos[2])
        )
...
class Gripper:
    ...
    def __init__(self, dc, ar):
        self.dc = dc
        self.ar = ar
        self.finger_j1 = self.dc.find_articulation_dof(self.ar, "right_finger_joint")
        self.finger_j2 = self.dc.find_articulation_dof(self.ar, "left_finger_joint")
        self.width = 0
...
class EndEffector:
    ...
    def look_at(self, gripper_pos, target):
        ...
        orientation = math_utils.lookAt(gripper_pos, target, (0, 0, 1))
...
class Dofbot:
    """Dofbot objects that contains implementation details for robot control
    """
    ...
    def __init__(self, stage, prim, dc, mp, world=None, default_config=None, is_ghost=False):
        ...
        self.prim = prim
        self.stage = stage
        # get handle to the articulation for this dofbot
        ...
        print("dofbot rmp handle", self.rmp_handle)
        ...
        print("end_effector", self.end_effector)
        if default_config:
