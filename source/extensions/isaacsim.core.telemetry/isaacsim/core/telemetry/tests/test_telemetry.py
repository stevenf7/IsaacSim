...
class TestTelemetryManager(omni.kit.test.AsyncTestCase):
    ...
    async def test_emit_extension_activated(self) -> None:
        ...
        result = emit_extension_activated("isaacsim.test.extension", "1.0.0", "enabled")
    ...
    async def test_emit_feature_used_without_duration(self) -> None:
        ...
        result = emit_feature_used("isaacsim.test.extension", "test_feature", "command")
        if self.manager.is_enabled():
            self.assertTrue(result)
        else:
            self.assertFalse(result)
    ...
    async def test_emit_feature_used_with_duration(self) -> None:
        ...
        result = emit_feature_used("isaacsim.test.extension", "test_feature", "api_call", duration_ms=42.5)
        if self.manager.is_enabled():
            self.assertTrue(result)
        else:
            self.assertFalse(result)

    async def test_emit_error_without_context(self) -> None:
        """Verify emit_error works without additional context."""
        result = emit_error("isaacsim.test.extension", "runtime_error", "Something failed")
        if self.manager.is_enabled():
            self.assertTrue(result)
        else:
            self.assertFalse(result)

    async def test_emit_error_with_context(self) -> None:
        """Verify emit_error works with additional context."""
        result = emit_error(
            "isaacsim.test.extension",
            "import_failure",
    ...
    async def test_send_event_unregistered_schema(self) -> None:
        ...
        result = self.manager.send_event("nonexistent.schema", "someEvent", {"key": "value"})
        self.assertFalse(result)
    ...
    async def test_register_custom_schema(self) -> None:
        ...
        events = self.manager.register_schema(schema_name, custom_schema)

        if self.manager.is_enabled():
            self.assertIsNotNone(events)
            self.assertIn("customEvent", events)

            result = self.manager.send_event(schema_name, "customEvent", {"testKey": "hello"})
            self.assertTrue(result)
        else:
            self.assertIsNone(events)

        self.manager.unregister_schema(schema_name)
    ...
    async def test_unregister_nonexistent_schema(self) -> None:
        ...
        self.manager.unregister_schema("does.not.exist")
