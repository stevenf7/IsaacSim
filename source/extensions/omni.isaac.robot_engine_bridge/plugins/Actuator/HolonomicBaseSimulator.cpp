...
void HolonomicBaseSimulator::tick()
{
    ...
    IsaacMessage<isaac_message::State> commandComposite;
    {
        ...
        if (checkErrorCode(receive(mInputComponent, mCommandChannelName, header, commandComposite, buffers)))
        {
            ...
            // Input comes in m/s
            mCommandedSpeed[0] =
                pxr::GfClamp(elements[0], double(-mMaximumSpeed[0]), double(mMaximumSpeed[0])) / mUnitScale;
            mCommandedSpeed[1] =
                pxr::GfClamp(elements[1], double(-mMaximumSpeed[0]), double(mMaximumSpeed[0])) / mUnitScale;
            mCommandedSpeed[2] = pxr::GfClamp(elements[2], double(-mMaximumSpeed[1]), double(mMaximumSpeed[1]));
