...
from .ui.UrdfJointWidgetWithID import UrdfJointWidget
from .ui.UrdfOptionWidget import UrdfOptionWidget

EXTENSION_NAME = "URDF Importer UI"
...
class UrdfImporter(object):
    def __init__(self, ext_id=None):
        ...
        # Get the core extension path for URDF data files
        core_ext_id = (
            omni.kit.app.get_app().get_extension_manager().get_enabled_extension_id("isaacsim.asset.importer.urdf")
        )
        self._core_extension_path = omni.kit.app.get_app().get_extension_manager().get_extension_path(core_ext_id)
    ...
    def _build_source_ui(self):
        ...
        with frame:
            kwargs = {
                ...
                "bookmark_path": f"{self._core_extension_path}/data/urdf",
    ...
    def _on_window(self, visible):
        if self._window.visible:
            ...
            self._stage_event_sub = carb.eventdispatcher.get_eventdispatcher().observe_event(
                ...
                observer_name="isaacsim.asset.importer.urdf.ui._on_stage_opened",
            )
        else:
            self._events = None
