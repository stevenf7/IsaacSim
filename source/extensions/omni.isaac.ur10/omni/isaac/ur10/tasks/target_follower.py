...
class TargetFollower(BaseTask):
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        self.target_cube = scene.add(
            VisualCube(
                ...
                position=np.array([0, 0.1, 0.7]) * 100,
    ...
    def add_obstacle(self, position: np.ndarray = np.array([0.1, 0.1, 1.0]) * 100):
        """[summary]
    ...
    def target_reached(self) -> bool:
        ...
        Returns:
            bool: [description]
        """
        end_effector_position, _ = self.my_ur10.end_effector.get_pose()
        target_cube_position, _ = self.target_cube.get_usd_pose()
        # TODO: change values with USD
        if np.mean(np.abs(np.array(end_effector_position) - np.array(target_cube_position))) < 0.05 * 100:
            return True
        else:
            return False
