...
class RMPFlowPickPlace(RMPFlowIKSolver):
    ...
    def __init__(self, name, mg_extension_path, dc_interface, stage, robot_prim, gripper_controller, gripper_length):
        super().__init__(
            name=name,
    ...
    def forward(self, cube_position, cube_orientation, cube_target_position, current_joint_positions):
        ...
        if self._event < 2:
            ...
            self._h0 = cube_position[2] + self._gripper_length
        ...
        if self._event == 2:
            ...
            target_joint_positions = super().forward(
                ...
                target_end_effector_orientation=euler_angles_to_quat(np.array([0, -np.pi, -np.pi / 2])),
            )
