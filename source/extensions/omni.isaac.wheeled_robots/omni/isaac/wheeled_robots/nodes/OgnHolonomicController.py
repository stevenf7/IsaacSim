...
class OgnHolonomicController:
    ...
    def compute(db) -> bool:
        ...
        try:
            ...
            if np.asarray((db.inputs.wheelPositions != state.wheel_positions)).any():
                state.wheel_positions = db.inputs.wheelPositions
                state.initialized = False

            if np.asarray((db.inputs.wheelOrientations != state.wheel_orientations)).any():
            ...
            if joint_positions is not None:
                db.outputs.jointPositionCommand = joint_positions
            if joint_velocities is not None:
                db.outputs.jointVelocityCommand = joint_velocities
            if joint_efforts is not None:
                db.outputs.jointEffortCommand = joint_efforts
