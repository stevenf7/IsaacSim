...
class Stacking:
    """Simplified stacking scene setup and controller using UR10Experimental and experimental APIs.
    ...
    def __init__(
        ...
        if cube_positions is None:
            ...
            cube_positions = [
                np.array([0.1, 0.1, cube_height_half]),
                np.array([0.1, -0.1, cube_height_half]),
            ]
    ...
    def setup_scene(self):
        ...
        self.robot = UR10Experimental(robot_path=self.robot_path, create_robot=True, attach_gripper=True)
        ...
        for i, cube_pos in enumerate(self.cube_positions):
            ...
            # Apply visual material (unique path to avoid clashes with other stackings)
            visual_material = PreviewSurfaceMaterial(f"/Visual_materials/ur10_{path_robot_name}_cube_{i}")
