...
The extension depends on `isaacsim.ros2.core` for the ROS 2 backend, `isaacsim.sensors.camera` and `isaacsim.sensors.physx` for sensor data access, and `omni.graph` for the OmniGraph framework. C++ publisher implementations for images and point clouds provide optimized data transfer for high-bandwidth sensor streams.