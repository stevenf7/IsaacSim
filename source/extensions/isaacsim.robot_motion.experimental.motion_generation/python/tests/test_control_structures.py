...
class BodyController(mg.BaseController):
    def reset(
        self, estimated_state: mg.RobotState, setpoint_state: Optional[mg.RobotState], t: float, **kwargs
    ) -> bool:
        return True
...
class ToolFrameController(mg.BaseController):
    def reset(
        self, estimated_state: mg.RobotState, setpoint_state: Optional[mg.RobotState], t: float, **kwargs
    ) -> bool:
        return True
...
class TestControlStructures(omni.kit.test.AsyncTestCase):
    ...
    async def test_controller_container(self):
        ...
        self.assertEqual(action.joints.names, ["default"])
        ...
        self.assertIsNotNone(action)
        self.assertEqual(action.joints.names, ["other"])
    ...
    async def test_parallel_controller(self):
        ...
        self.assertEqual(desired_state.joints.names, ["default", "other"])
        ...
        # combining root, body, and tool frame outputs in parallel is valid:
        parallel_controller = mg.ParallelController(
            controllers=[RootController(), BodyController(), ToolFrameController()],
        )
        desired_state = parallel_controller.forward(empty_robot_state, None, 0.0)
        self.assertIsNotNone(desired_state)
    ...
    async def test_sequential_controller(self):
        ...
        self.assertIsNotNone(desired_state)
        self.assertEqual(desired_state.joints.names, ["default"])
