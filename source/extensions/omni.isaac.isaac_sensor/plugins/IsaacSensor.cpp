...
const struct carb::PluginImplDesc kPluginImpl = { "omni.isaac.isaac_sensor.plugin", "Isaac Contact Sensor", "NVIDIA",
                                                  carb::PluginHotReload::eDisabled, "dev" };


CARB_PLUGIN_IMPL(kPluginImpl,
                 omni::isaac::isaac_sensor::ContactSensorInterface,
                 omni::isaac::isaac_sensor::ImuSensorInterface)
...
std::unique_ptr<omni::isaac::isaac_sensor::IsaacSensorManager> gIsaacSensorManager;
...
bool CARB_ABI isImuSensor(const char* primPath)
{
    if (g_stage && gIsaacSensorManager)
    {
        omni::isaac::isaac_sensor::ImuSensor* sensor =
...
omni::isaac::isaac_sensor::IsReading* CARB_ABI IsGetSensorReadings(const char* primPath, size_t& num_readings)
{
    omni::isaac::isaac_sensor::ImuSensor* sensor =
        gIsaacSensorManager->getImuSensor(g_stage->GetPrimAtPath(pxr::SdfPath(primPath)));
    omni::isaac::isaac_sensor::IsReading* data = nullptr;
    if (sensor)
    {
        data = sensor->getSensorReadings(num_readings);
    }
...
omni::isaac::isaac_sensor::IsReading CARB_ABI IsGetSensorSimReading(const char* primPath)
{
    omni::isaac::isaac_sensor::ImuSensor* sensor =
        gIsaacSensorManager->getImuSensor(g_stage->GetPrimAtPath(pxr::SdfPath(primPath)));
    omni::isaac::isaac_sensor::IsReading data;
    if (sensor)
    {
        data = sensor->getSimSensorReading();
    }
...
CARB_EXPORT void carbOnPluginStartup()
{
    ...
    gIsaacSensorManager = std::make_unique<omni::isaac::isaac_sensor::IsaacSensorManager>(g_physx, g_debugDraw);
...
void fillInterface(omni::isaac::isaac_sensor::ImuSensorInterface& iface)
{
    using namespace omni::isaac::isaac_sensor;
