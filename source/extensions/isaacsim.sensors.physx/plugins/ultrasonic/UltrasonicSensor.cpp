...
void UltrasonicSensor::onComponentChange()
{
    ...
    omni::isaac::utils::safeGetAttribute(typedPrim.GetNumBinsAttr(), mNumBins);
    omni::isaac::utils::safeGetAttribute(typedPrim.GetUseBRDFAttr(), mUseBRDF);
    omni::isaac::utils::safeGetAttribute(typedPrim.GetUseUSSMaterialsForBRDFAttr(), mUseUSSMaterialsForBRDF);
    ...
    mAdjacency = isaacsim::sensors::physx::extractAdjacencyVectors(mEmitters);
}
...
void UltrasonicSensor::tick()
{
    ...
    if (mFiringGroups.size() > 0)
    {
        ...
        std::vector<::physx::PxTransform> origins = isaacsim::sensors::physx::extractOrigins(mEmitters);
