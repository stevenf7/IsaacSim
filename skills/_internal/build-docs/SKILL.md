...
## Docs Image / Video Filename Convention

**Every** image or video added under `docs/isaacsim/images/` (including icons,
diagrams, screenshots, generated assets, programmatically-created PNGs, etc.)
must follow this pattern, otherwise the `check-docs-image-filenames` MR job
fails:
...
|------------------|-----------------------------------------------------------------------------|
| `VERSION_NUM`    | `x.y` or `x.y.z` (use the current Isaac Sim release, e.g. `6.0`)            |
| `APP_TYPE`       | `base`, `full`, `isaaclab`, `replicator`, `ros`                             |
| `DOC_TYPE`       | `ref`, `tut`, or `ext-<extension_name>-<x.y.z>`                             |
| `APP_VIEW`       | `external`, `gui`, `viewport`                                               |
| `YOUR_FILE_NAME` | Non-empty descriptive snake_case text; may itself contain underscores       |
| `ext`            | `png`, `jpg`, `jpeg`, `gif`, `webp`, `svg`, `mp4`, `webm`, `mov`, `mkv`, ...|

Common assignments:

- Reference/conceptual icons or diagrams that are not viewport captures:
  `isim_6.0_base_ref_gui_<descriptive_name>.png`
- Viewport captures generated by `repo_capture` or similar tooling:
  `isim_6.0_full_ref_viewport_<descriptive_name>.png`
- Tutorial screenshots: `isim_6.0_full_tut_gui_<descriptive_name>.png`
- Extension-scoped media:
  `isim_6.0_full_ext-isaacsim.ros2.bridge-3.0.0_viewport_<descriptive_name>.gif`

When you create or rename a docs image, validate the filename **before**
referencing it in any RST/MD:

```bash
echo "<relative/path/to/new_image.png>" > /tmp/files.txt
python3 docs/tools/validate_filenames/validate_filenames.py --files-from /tmp/files.txt
```

If the validator rejects the name, rename until it passes, then update every
RST/MD reference. The full check against the target branch (mirrors the MR
job) is shown below.
...
- `check-docs-image-filenames` validates changed docs image/video filenames against the Isaac Sim naming convention above.
...
If `check-docs-image-filenames` fails, rename the changed media file to the required pattern and update every RST/MD reference.
