...
def setup_repo_tool(parser: argparse.ArgumentParser, config: Dict) -> Callable:
    parser.description = "Update extensions cache (isaacsim.exp.extscache.kit)."
    parser.add_argument(
        "-o",
        "--offline",
        dest="offline",
    ...
    def run_repo_tool(options: Dict, config: Dict):
        print(
            'Please review the output of this script at ./source/apps/isaacsim.exp.extscache.kit before committing changes to the repo. Run "./build.sh -u -r" to update the cache. See the results of the cache update at the bottom of the isaacsim.exp.extscache.kit file. To make changes to the configuration of this script, edit ./repo.toml.'
        )
        ...
        # Used to store the new text that will be written to isaacsim.exp.extscache.kit
        output = []

        # If there are dependencies currently in isaacsim.exp.extscache.kit that were manually added to the top of the file, keep them
        for line in dest_file_lines:
            output.append(line)
            line = line.strip()
        ...
        # Write the output text to isaacsim.exp.extscache.kit
...
# Print a dependency in the format used by isaacsim.exp.extscache.kit --> "extension.name" = {version = "version.number", exact = true}, or "extension.name" = {tag = "tag_name", version = "version.number", exact = true}
