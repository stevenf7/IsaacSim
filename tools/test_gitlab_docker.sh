...
nvidia-docker run -it --entrypoint bash --gpus all -e "ACCEPT_EULA=Y" --rm --network=host -v /tmp/.X11-unix:/tmp/.X11-unix -v /etc/localtime:/etc/localtime:ro -e DISPLAY=unix${DISPLAY} gitlab-master.nvidia.com:5005/isaac/omni_isaac_sim/isaac-sim:latest-develop