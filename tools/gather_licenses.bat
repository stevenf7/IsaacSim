...
call "%~dp0licensing.bat" gather -d "%cd%" -p "deps\rtx-plugins.packman.xml" --platform "windows-x86_64" %LICENSING_OPTIONS% %*
if %errorlevel% neq 0 ( goto End )

call "%~dp0licensing.bat" gather -d "%cd%" -p "deps\omni-physics.packman.xml" --platform "windows-x86_64" %LICENSING_OPTIONS% %*
if %errorlevel% neq 0 ( goto End )
