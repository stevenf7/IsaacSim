...
class runtime:
  ...
  pythons = { 'windows' : os.path.join(packman_path, "python.bat"),     
              'linux'   : os.path.join(packman_path, "python.sh") }
...
def bootstrap(runtime_dir=None, 
              pip_requirements_file=None, 
              packman_project_file=None,
              subdirs={}):
  ...
  runtime.env_config_file_path = os.path.join(runtime_dir, 
                                              runtime.env_config_file_name)
  ...
  if(os.getenv(opts.load_prepped_env_env_var, None)):
    ...
    if(packman_project_file):
      pm_paths = _bootstrap_packman_deps(_rel_to_bin(packman_project_file), 
                                         subdirs=subdirs,
                                         make_links=prep_env_only_flag)
...
def _bootstrap_prepared_env(subdirs={}):
  ...
  if(os.path.isfile(runtime.env_config_file_path)):
    _err_out(f"  > loading additional env info from " + 
             f"{runtime.env_config_file_path}")
...
def _bootstrap_packman():
  ...
  ### NOTE -- as of Packman 7.x, this seems to be unnecessary
#  ret = subprocess.call([packman, "pull", f"{runtime.deps['packman-common']}/deps.packman.xml"])
#  if(ret):
#    _err_out(f"Downloading packman base deps failed with {ret}")
#    sys.exit(ret)

#  seven_z_path = os.path.join(os.environ['PM_PACKAGES_ROOT'], 'chk', '7za', '16.02.4')
#  if(not os.path.exists(seven_z_path)):
#    _err_out(f"7-Zip package did not appear at {seven_z_path}: something's horribly wrong")
#    sys.exit(ret)

#  os.environ['PM_7za_PATH'] = seven_z_path
  ## -- END NOTE 
...
def _bootstrap_packman_deps(projectf, subdirs={}, make_links=0):
  ...
  pull_result = packmanapi.pull(project_path=projectf, 
                                platform=platform)
    ...
    if(dep in subdirs):
      ...
      for subdir in _to_arr_if_scal(subdirs[dep]):
        ...
        if not(os.path.isdir(path)):
          _err_out(f"Subdir `{subdirs[dep]}` does not exist at `{location}`, " +
                   f"downloaded by packman")
          sys.exit(1)
        else:
          paths.append(path)
      del subdirs[dep]
  ...
  if(len(subdirs)):
    for (dep, subdir) in subdirs.items():
      _err_out(f"  > you requested to add `{subdir}` subdir of `{dep}`, " +     
...
def _bootstrap_pip_deps(reqf):
  ...
  if(_already_bootstrapped(reqf)):
    _err_out(f"  > looks like already bootstrapped") 
     ...
     ret = subprocess.call([sys.executable, "-m", "pip", 
                            "--isolated", 
                            "install", 
                            "--target", runtime.deps['pip'],
                            "-r", reqf])
     if(ret):
...
def _setup_dep_dirs(runtime_dir):
  for subdir in runtime.deps.keys():
    ...
    # Packman likes to create it's directory itself. 
    if(subdir == 'packman-common'):
      continue
