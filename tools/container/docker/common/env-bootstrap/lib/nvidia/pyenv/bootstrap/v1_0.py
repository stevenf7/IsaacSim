...
def bootstrap(runtime_dir=None, 
  ...
  if( not(os.getenv(opts.relaunch_flag_env_var, None)) and
      not(os.getenv(opts.no_relaunch_flag_env_var, None)) ):
  ...
  if(os.getenv(opts.load_prepped_env_env_var, None)):
    _bootstrap_prepared_env(subdirs=subdirs)
  else:
...
def _bootstrap_prepared_env(subdirs={}):
  ...
  if(os.path.isfile(runtime.env_config_file_path)):
    ...
    _load_and_process_env_file(runtime.env_config_file_path, subdirs=subdirs)
...
def _bootstrap_pip_deps(reqf):
  ...
  _err_out(f"  > to: {runtime.deps['pip']}")

  with open(reqf, 'r') as h: 
    ...
    while(1):
      ...
      if(line.find('==') != -1):
        ...
        _err_out(" !!! PIP specs using `>` are not supported !!!")
        sys.exit(1)
      else:
        _err_out("    > WARNING: no version specified")
        pkg = line

      _repoman.pip_install(pkg, runtime.deps['pip'], version)
